...
class AuthServiceTest {
    ...
    @Test
    void invalidTokenDoesNotMasqueradeAsAdminInIdentityView() {
        PlatformProperties properties = new PlatformProperties();
        properties.getSecurity().setEnabled(true);
        AuthService service = new AuthService(properties, new PlatformStore());
    ...
    @Test
    void configuredAdminUsernameIsComparedIgnoringCaseAndWhitespace() {
        PlatformProperties properties = new PlatformProperties();
        properties.getSecurity().setEnabled(true);
        ...
        assertTrue(service.permissionsForToken(session.token()).contains("SYSTEM_SETTINGS"));
    }

    @Test
    void builtInAdminKeepsAllPermissionsWhenConfiguredNameDiffers() {
        PlatformProperties properties = new PlatformProperties();
        properties.getSecurity().setEnabled(true);
        properties.getSecurity().setAdminUsername("platform-admin");
        PlatformStore store = new PlatformStore();
        store.users.put(1L, new UserView(1L, "admin", "平台管理员", "ACTIVE", sha256("secret")));
    ...
    @Test
    void legacyStoredHashIsUpgradedAfterSuccessfulLogin() {
        PlatformProperties properties = new PlatformProperties();
        properties.getSecurity().setEnabled(true);
        PlatformStore store = new PlatformStore();
        store.users.put(7L, new UserView(7L, "legacy", "Legacy", "USER", "ACTIVE", java.time.LocalDateTime.now(), sha256("secret")));
        AuthService service = new AuthService(properties, store);
        service.login(new AuthRequests.LoginRequest("legacy", "secret"));
        assertTrue(store.users.get(7L).passwordHash().startsWith("pbkdf2-sha256$"));
    }

    @Test
    void adminCanChangePasswordAndLoginWithTheNewPassword() {
        PlatformProperties properties = new PlatformProperties();
        ...
        AuthService.AuthSession next = service.login(new AuthRequests.LoginRequest("admin", "new-secret"));
        assertEquals("admin", next.username());
    }

    @Test
    void changingConfiguredAdminPasswordDisablesTheOldBootstrapPasswordAndInvalidatesSession() {
        PlatformProperties properties = new PlatformProperties();
        properties.getSecurity().setEnabled(true);
        properties.getSecurity().setAdminPasswordHash(sha256("old-secret"));
        PlatformStore store = new PlatformStore();
        AuthService service = new AuthService(properties, store);
    ...
    @Test
    void permissionChecksIncludeHttpMethodForDataSourcesAndLegacyDevelopmentAliases() {
        PlatformProperties properties = new PlatformProperties();
        properties.getSecurity().setEnabled(true);
        PlatformStore store = new PlatformStore();
        UserView user = new UserView(7L, "analyst", "Analyst", "USER", "ACTIVE", java.time.LocalDateTime.now(), sha256("secret"));
        store.users.put(user.id(), user);
        store.userPermissions.put(user.id(), java.util.concurrent.ConcurrentHashMap.newKeySet());
        store.userPermissions.get(user.id()).add("DATA_EXPLORE");
        AuthService service = new AuthService(properties, store);
