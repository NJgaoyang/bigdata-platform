...
public class PlatformStore {
    ...
    public void loadPersistedCoreData() {
        ...
        try {
            ...
            jdbc.query("SELECT id,name,source_type,target_type,sync_mode,status,seatunnel_config FROM integration_task", rs -> {
                long id = rs.getLong("id");
                integrationTasks.put(id, new IntegrationTaskView(id, rs.getString("name"), rs.getString("source_type"),
                        rs.getString("target_type"), rs.getString("sync_mode"), rs.getString("status"), rs.getString("seatunnel_config")));
                advanceId(id);
            });
            ...
            jdbc.query("SELECT id,workflow_id,node_name,node_type,file_version_id,config_json,x,y FROM workflow_node", rs -> {
                ...
                loadedNodes.computeIfAbsent(workflowId, ignored -> new ArrayList<>()).add(new WorkflowNodeView(
                        ...
                        rs.getObject("file_version_id", Long.class), rs.getString("config_json"), rs.getInt("x"), rs.getInt("y")));
            ...
            jdbc.query("SELECT id,username,display_name,status FROM platform_user", rs -> {
                long id = rs.getLong("id");
                users.put(id, new UserView(id, rs.getString("username"), rs.getString("display_name"), rs.getString("status")));
                advanceId(id);
            });
            ...
            jdbc.query("SELECT id,workflow_id,cron_expression,timezone,enabled,failure_strategy FROM schedule_config", rs -> {
                long id = rs.getLong("id");
                scheduleConfigs.put(id, new ScheduleConfigView(id, rs.getLong("workflow_id"), rs.getString("cron_expression"),
                        rs.getString("timezone"), rs.getBoolean("enabled"), rs.getString("failure_strategy"), 1));
                advanceId(id);
            });
    ...
    public void persistVersion(FileVersionView view) {
        if (jdbc == null) return;
        int updated = jdbc.update("UPDATE dev_file_version SET content=?,checksum=?,publish_flag=? WHERE id=?",
                view.content(), view.checksum(), view.publishFlag(), view.id());
    ...
    public void persistIntegrationTask(IntegrationTaskView task) {
        if (jdbc == null) return;
        int updated = jdbc.update("UPDATE integration_task SET name=?,source_type=?,target_type=?,sync_mode=?,status=?,seatunnel_config=?,updated_at=CURRENT_TIMESTAMP WHERE id=?",
                task.name(), task.sourceType(), task.targetType(), task.syncMode(), task.status(), task.seatunnelConfig(), task.id());
        if (updated == 0) jdbc.update("INSERT INTO integration_task (id,name,source_type,target_type,source_config_json,target_config_json,sync_mode,status,seatunnel_config) VALUES (?,?,?,?,?,?,?,?,?)",
                task.id(), task.name(), task.sourceType(), task.targetType(), "{}", "{}", task.syncMode(), task.status(), task.seatunnelConfig());
    }
    ...
    public void persistWorkflow(WorkflowView workflow) {
        ...
        for (WorkflowNodeView node : workflow.nodes()) {
            jdbc.update("INSERT INTO workflow_node (id,workflow_id,node_code,node_name,node_type,file_version_id,config_json,x,y) VALUES (?,?,?,?,?,?,?,?,?)",
                    node.id(), workflow.id(), "node_" + node.id(), node.name(), node.nodeType().name(), node.fileVersionId(), node.configJson(), node.x(), node.y());
        }
    ...
    public void persistSchedule(ScheduleConfigView schedule) {
        if (jdbc == null) return;
        int updated = jdbc.update("UPDATE schedule_config SET workflow_id=?,cron_expression=?,timezone=?,enabled=?,failure_strategy=?,updated_at=CURRENT_TIMESTAMP WHERE id=?",
                schedule.workflowId(), schedule.cronExpression(), schedule.timezone(), schedule.enabled(), schedule.failureStrategy(), schedule.id());
        if (updated == 0) jdbc.update("INSERT INTO schedule_config (id,workflow_id,cron_expression,timezone,enabled,failure_strategy) VALUES (?,?,?,?,?,?)",
                schedule.id(), schedule.workflowId(), schedule.cronExpression(), schedule.timezone(), schedule.enabled(), schedule.failureStrategy());
    }
    public void persistUser(UserView user) {
        if (jdbc == null) return;
        int updated = jdbc.update("UPDATE platform_user SET username=?,display_name=?,status=? WHERE id=?", user.username(), user.displayName(), user.status(), user.id());
        if (updated == 0) jdbc.update("INSERT INTO platform_user (id,username,display_name,status) VALUES (?,?,?,?)", user.id(), user.username(), user.displayName(), user.status());
    }
    ...
    private void refreshIdSequence() {
        ...
        for (String table : List.of("data_source", "dev_project", "dev_folder", "dev_file", "dev_file_version",
                ...
                "platform_user", "platform_role", "alert_channel", "operation_audit", "schedule_config")) {
            try {
