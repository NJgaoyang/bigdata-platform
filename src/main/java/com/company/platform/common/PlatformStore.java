...
public class PlatformStore {
    ...
    public void loadPersistedCoreData() {
        ...
        try {
            ...
            jdbc.query("SELECT id,workflow_id,node_code,node_name,node_type,dev_file_id,config_json,x,y FROM workflow_node", rs -> {
                ...
                loadedNodes.computeIfAbsent(workflowId, ignored -> new ArrayList<>()).add(new WorkflowNodeView(
                        ...
                        rs.getObject("dev_file_id", Long.class), rs.getString("config_json"),
    ...
    public void persistFile(DevFileView view) {
        ...
        int updated = jdbc.update("UPDATE dev_file SET project_id=?,folder_id=?,name=?,file_type=?,content=?,description=?,status=?,current_version=?,lifecycle_status=?,ever_online=?,owner_name=?,updated_at=CURRENT_TIMESTAMP WHERE id=?",
                view.projectId(), view.folderId(), view.name(), view.fileType(), view.content(), view.description(), view.status(), view.currentVersion(), view.lifecycleStatus(), view.everOnline(), view.ownerName(), view.id());
    ...
    public void persistWorkflow(WorkflowView workflow) {
        ...
        for (WorkflowNodeView node : workflow.nodes()) {
            jdbc.update("INSERT INTO workflow_node (id,workflow_id,node_code,node_name,node_type,dev_file_id,config_json,x,y) VALUES (?,?,?,?,?,?,?,?,?)",
