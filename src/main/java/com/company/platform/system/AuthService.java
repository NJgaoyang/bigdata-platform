...
public class AuthService {
    ...
    private boolean isConfiguredAdmin(String username) {
        ...
        return normalizedUsername.equalsIgnoreCase("admin") || configuredAdmin != null && normalizedUsername.equalsIgnoreCase(configuredAdmin.trim());
    }
    ...
    public String roleForToken(String token) {
        ...
        return store.users.values().stream().filter(item -> item.username().equalsIgnoreCase(username)).map(UserView::roleCode).findFirst().orElse("USER");
    }
    public boolean isSuperAdminToken(String token) { String username = currentUsername(token); return !username.isBlank() && isConfiguredAdmin(username); }
