...
const filteredAudits = computed(() => {
  ...
  return audits.value.filter(item => !query || [item.operatorName, item.action, item.resourceType, item.detail, auditAction(item.action), auditObject(item), auditDetail(item)].some(value => String(value || "").toLowerCase().includes(query)));
});
...
const auditActionLabels: Record<string, string> = {
  API_REQUEST: "系统操作", ACCESS_DENIED: "访问拒绝", LOGIN: "用户登录", LOGOUT: "退出登录", LOGIN_FAILED: "登录失败",
...
async function toggleUser(user: User) {
  ...
  try {
    ...
    ElMessage.error(error?.response?.data?.message || "用户状态更新失败");
  }
}
...
async function saveSource() {
  try {
    if (editingSourceId.value) await platformApi.updateDataSource(editingSourceId.value, sourceForm.value);
    else await platformApi.createDataSource(sourceForm.value);
    ...
    ElMessage.error(error?.response?.data?.message || "数据源保存失败");
  }
}
...
async function testSource(source: DataSource) {
  try {
    ...
    ElMessage.error(error?.response?.data?.message || "数据源连接测试失败");
  }
}
...
async function saveCluster() {
  try {
    ...
    ElMessage.error(error?.response?.data?.message || "客户端保存失败");
  }
}
    ...
    <div class="settings-layout">
      ...
      <main class="settings-content">
        ...
        <template v-else-if="section === 'audits'">
          ...
          <div class="card audit-searchbar"><el-icon><Search /></el-icon><input v-model="auditKeyword" placeholder="搜索用户、操作、对象或详情"><button v-if="auditKeyword" class="audit-search-clear" @click="auditKeyword = ''">清除</button><span class="audit-count">{{ filteredAudits.length }} 条记录</span></div>
...
<style scoped>
.settings-layout { display:flex; min-height:calc(100vh - 112px); background:#f7f9fc; }
.settings-nav { width:270px; flex:0 0 270px; padding:12px 18px 24px; border-right:1px solid #dfe7f0; background:#fff; }
.settings-nav-item { display:flex; align-items:center; gap:14px; width:100%; min-height:58px; margin:2px 0; padding:10px 18px; border:1px solid transparent; border-radius:8px; text-align:left; color:#667085; background:transparent; font:inherit; font-size:12px; font-weight:400; line-height:1.5; letter-spacing:normal; cursor:pointer; transition:background .18s ease, color .18s ease, border-color .18s ease; }
...
.settings-nav-label { font-family:inherit; font-size:12px; font-weight:400; line-height:inherit; letter-spacing:normal; color:inherit; }
.settings-nav-icon { display:inline-flex; align-items:center; justify-content:center; width:36px; height:36px; border-radius:7px; color:#667085; background:#f2f4f7; font-size:20px; flex:0 0 36px; }
.settings-nav-item.active .settings-nav-icon { color:#344054; background:#e4e7ec; }
.settings-content { flex:1; min-width:0; padding:26px 30px; }
...
.audit-heading { margin-bottom:24px; }.audit-heading h2 { font-size:24px; font-weight:700; letter-spacing:-.02em; }.audit-heading p { margin-top:7px; font-size:15px; color:#566b89; }
.audit-searchbar { display:flex; align-items:center; gap:12px; min-height:76px; margin-bottom:16px; padding:14px 20px; border-color:#dbe5f0; border-radius:14px; }.audit-searchbar :deep(.el-icon) { color:#8294ad; font-size:19px; }.audit-searchbar input { flex:1; min-width:0; height:42px; border:0; outline:0; color:#344054; background:transparent; font:inherit; font-size:14px; }.audit-searchbar input::placeholder { color:#98a7bb; }.audit-search-clear { border:0; color:#667085; background:transparent; font:inherit; font-size:13px; cursor:pointer; }.audit-search-clear:hover { color:#1677ff; }.audit-count { padding-left:14px; border-left:1px solid #e5ebf2; color:#8493a7; font-size:13px; white-space:nowrap; }
.audit-table th:nth-child(1) { width:7%; }.audit-table th:nth-child(2) { width:10%; }.audit-table th:nth-child(3) { width:13%; }.audit-table th:nth-child(4) { width:23%; }.audit-table th:nth-child(5) { width:20%; }.audit-table th:nth-child(6) { width:8%; }.audit-table th:nth-child(7) { width:19%; }
