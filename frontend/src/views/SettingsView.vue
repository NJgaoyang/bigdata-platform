...
import { platformApi, type DataSource, type DolphinSchedulerCluster, type SeaTunnelCluster } from "../api";
...
const clusterTab = ref<"seatunnel" | "dolphinscheduler">("seatunnel");
...
const dolphinClusters = ref<DolphinSchedulerCluster[]>([]);
const dolphinDialog = ref(false);
const editingDolphinId = ref<number | null>(null);
const dolphinForm = ref({ name: "", host: "", port: 12345, basePath: "/dolphinscheduler", version: "3.1.9", username: "admin", password: "", installDir: "/data/software/dolphinscheduler", description: "" });
const dolphinDialogTitle = computed(() => editingDolphinId.value ? "编辑 DolphinScheduler 集群" : "新增 DolphinScheduler 集群");
...
async function loadDolphinClusters() {
  try {
    dolphinClusters.value = (await platformApi.dolphinSchedulerClusters()).data.data || [];
  } catch (error) {
    dolphinClusters.value = [];
    ElMessage.error(errorMessage(error, "DolphinScheduler 集群加载失败"));
  }
}
function openCreateDolphinCluster() {
  editingDolphinId.value = null;
  dolphinForm.value = { name: "", host: "", port: 12345, basePath: "/dolphinscheduler", version: "3.1.9", username: "admin", password: "", installDir: "/data/software/dolphinscheduler", description: "" };
  dolphinDialog.value = true;
}
function openEditDolphinCluster(cluster: DolphinSchedulerCluster) {
  editingDolphinId.value = cluster.id;
  dolphinForm.value = { name: cluster.name, host: cluster.host, port: cluster.port, basePath: cluster.basePath || "/dolphinscheduler", version: cluster.version || "3.1.9", username: cluster.username || "admin", password: "", installDir: cluster.installDir || "", description: cluster.description || "" };
  dolphinDialog.value = true;
}
async function saveDolphinCluster() {
  const form = dolphinForm.value;
  if (![form.name, form.host, form.basePath].every(item => item.trim())) {
    ElMessage.warning("请填写 DolphinScheduler 集群名称、主机地址和访问路径");
    return;
  }
  ...
  try {
    if (editingDolphinId.value) await platformApi.updateDolphinSchedulerCluster(editingDolphinId.value, form);
    else await platformApi.createDolphinSchedulerCluster(form);
    dolphinDialog.value = false;
    await loadDolphinClusters();
    ElMessage.success(editingDolphinId.value ? "DolphinScheduler 集群已更新" : "DolphinScheduler 集群已创建");
  } catch (error) {
    ElMessage.error(errorMessage(error, "DolphinScheduler 集群保存失败"));
  }
}
async function checkDolphinCluster(cluster: DolphinSchedulerCluster) {
  try {
    const result = await platformApi.checkDolphinSchedulerCluster(cluster.id);
    const updated = result.data.data;
    dolphinClusters.value = dolphinClusters.value.map(item => item.id === updated.id ? updated : item);
    ElMessage.success(updated.healthStatus === "HEALTHY" ? "DolphinScheduler 服务连接正常" : "DolphinScheduler 服务暂不可达");
  } catch (error) {
    ElMessage.error(errorMessage(error, "DolphinScheduler 集群检查失败"));
  }
}
async function checkAllDolphinClusters() {
  try {
    dolphinClusters.value = (await platformApi.checkAllDolphinSchedulerClusters()).data.data || [];
    ElMessage.success("DolphinScheduler 集群检查已完成");
  } catch (error) {
    ElMessage.error(errorMessage(error, "DolphinScheduler 集群检查失败"));
  }
}
async function deleteDolphinCluster(cluster: DolphinSchedulerCluster) {
  if (!window.confirm(`确认删除 DolphinScheduler 集群“${cluster.name}”吗？`)) return;
  try {
    await platformApi.deleteDolphinSchedulerCluster(cluster.id);
    dolphinClusters.value = dolphinClusters.value.filter(item => item.id !== cluster.id);
    ElMessage.success("DolphinScheduler 集群已删除");
  } catch (error) {
    ElMessage.error(errorMessage(error, "DolphinScheduler 集群删除失败"));
  }
}
function schedulerHealthLabel(value: string) { return value === "HEALTHY" ? "健康" : value === "UNREACHABLE" ? "不可达" : "未检查"; }
...
async function changeSection(next: Section) {
  ...
  if (next === "clusters") await Promise.all([loadClusters(), loadDolphinClusters()]);
}
onMounted(async () => {
  await Promise.allSettled([loadUsers(), loadAudits(), loadClusters(), loadDolphinClusters(), platformApi.dataSources().then(result => { sources.value = result.data.data || []; })]);
});
</script>
  ...
  <section class="page">
    ...
    <div class="settings-layout">
      ...
      <main class="settings-content">
        ...
        <template v-else>
          <div class="section-heading cluster-heading"><div><h2>集群配置</h2><p>分别管理 SeaTunnel 和 DolphinScheduler 集群服务</p></div></div>
          <div class="cluster-tabs"><button :class="{ active: clusterTab === 'seatunnel' }" @click="clusterTab = 'seatunnel'">SeaTunnel 集群</button><button :class="{ active: clusterTab === 'dolphinscheduler' }" @click="clusterTab = 'dolphinscheduler'">DolphinScheduler 集群</button></div>
          <template v-if="clusterTab === 'seatunnel'">
          ...
          <template v-else>
            <div class="cluster-toolbar"><button class="btn-primary" @click="openCreateDolphinCluster">＋ 新增集群</button><button class="btn-default" @click="checkAllDolphinClusters">⟳ 检查全部</button></div>
            <div class="card cluster-table-card"><table class="data-table cluster-table scheduler-cluster-table"><thead><tr><th>名称</th><th>服务地址</th><th>版本</th><th>节点健康</th><th>描述</th><th>操作</th></tr></thead><tbody><tr v-for="cluster in dolphinClusters" :key="cluster.id"><td class="cluster-name">{{ cluster.name }}</td><td>{{ cluster.host }}:{{ cluster.port }}{{ cluster.basePath }}</td><td>{{ cluster.version }}</td><td><span class="cluster-health" :class="cluster.healthStatus.toLowerCase()">{{ schedulerHealthLabel(cluster.healthStatus) }}</span></td><td>{{ cluster.description || '—' }}</td><td class="cluster-actions"><button class="user-action" @click="checkDolphinCluster(cluster)">检查</button><button class="user-action" @click="openEditDolphinCluster(cluster)">编辑</button><button class="user-action delete" @click="deleteDolphinCluster(cluster)">删除</button></td></tr><tr v-if="!dolphinClusters.length"><td colspan="6" class="empty-state">暂无 DolphinScheduler 集群，请新增集群</td></tr></tbody></table></div>
          </template>
        </template>
      </main>
    </div>
    ...
    <div v-if="dolphinDialog" class="modal-mask cluster-modal-mask" @click.self="dolphinDialog = false"><div class="modal cluster-modal"><div class="modal-head cluster-modal-head"><span>{{ dolphinDialogTitle }}</span><button class="modal-close" aria-label="关闭" @click="dolphinDialog = false">×</button></div><div class="modal-body cluster-modal-body"><div class="cluster-form-item"><label>集群名称</label><input v-model="dolphinForm.name" placeholder="例如 生产调度集群"></div><div class="cluster-form-grid"><div class="cluster-form-item"><label>主机地址</label><input v-model="dolphinForm.host" placeholder="例如 81.69.15.136"></div><div class="cluster-form-item"><label>端口</label><input v-model.number="dolphinForm.port" type="number"></div></div><div class="cluster-form-grid"><div class="cluster-form-item"><label>访问路径</label><input v-model="dolphinForm.basePath" placeholder="/dolphinscheduler"></div><div class="cluster-form-item"><label>版本</label><input v-model="dolphinForm.version" placeholder="3.1.9"></div></div><div class="cluster-form-grid"><div class="cluster-form-item"><label>登录用户</label><input v-model="dolphinForm.username" placeholder="admin"></div><div class="cluster-form-item"><label>登录密码</label><input v-model="dolphinForm.password" type="password" placeholder="可选"></div></div><div class="cluster-form-item"><label>安装目录</label><input v-model="dolphinForm.installDir" placeholder="/data/software/dolphinscheduler"></div><div class="cluster-form-item"><label>描述</label><textarea v-model="dolphinForm.description" rows="3" placeholder="补充集群用途或环境说明"></textarea></div></div><div class="modal-foot cluster-modal-foot"><button class="btn-default" @click="dolphinDialog = false">取消</button><button class="btn-primary" @click="saveDolphinCluster">保存</button></div></div></div>
...
.cluster-modal { width:min(640px, calc(100vw - 32px)); }
</style>
