        ...
        <div class="tree">
          <div v-for="db in visibleDatabases" :key="db.name" :class="['tree-db',{expanded:expanded.has(db.name)}]">
            <button :class="['tree-db-row',{on:database===db.name&&!table}]" @click="toggleDatabase(db.name)">
              <span class="tree-chevron">›</span><span class="tree-icon">DB</span>
              <span class="tree-main"><strong class="tree-title">{{db.name}}</strong><small class="tree-sub">{{db.comment||`${(tableCache[db.name]||[]).length || '—'} 个数据表`}}</small></span>
            </button>
            <div v-if="expanded.has(db.name)" class="tree-children">
              <button v-for="t in visibleTables(db.name)" :key="t.name" :class="['tree-table-row',{on:database===db.name&&table?.name===t.name}]" @click="selectTable(db.name,t)">
                <span class="tree-icon table-icon">T</span><span class="tree-main"><strong class="tree-title">{{t.name}}</strong><small class="tree-sub">{{t.comment||t.type||'数据表'}}</small></span>
              </button>
