...
<template><div class="ds-page">
  ...
  <div v-else-if="mode==='users'" class="ds-card"><table class="ds-table"><thead><tr><th>真实姓名</th><th>账号</th><th>手机号</th><th>角色</th><th>状态</th><th>创建时间</th><th>操作</th></tr></thead><tbody><tr v-for="u in users" :key="u.id"><td class="ds-resource">{{u.displayName}}</td><td>{{u.username}}</td><td>{{u.phone||'—'}}</td><td>{{userRoleName(u.roleCode)}}</td><td><el-switch :model-value="u.status==='ACTIVE'" :disabled="!canEdit" @change="toggle(u)"/></td><td>{{fmt(u.createdAt)}}</td><td><template v-if="canEdit"><span class="ds-link" @click="openEditUser(u)">编辑</span><span class="sep">·</span><span class="danger" @click="remove(u)">删除</span></template><span v-else>—</span></td></tr></tbody></table></div>
  ...
  <el-dialog v-model="userDialog" :title="editingUser?'编辑用户':'新增用户'" width="680px"><el-form label-position="top" class="user-form-grid"><el-form-item label="账号"><el-input v-model="userForm.username" placeholder="请输入登录账号"/></el-form-item><el-form-item label="真实姓名"><el-input v-model="userForm.displayName" placeholder="请输入真实姓名"/></el-form-item><el-form-item :label="editingUser?'重置密码':'初始密码'"><el-input v-model="userForm.password" type="password" show-password :placeholder="editingUser?'留空表示不修改密码':'请输入初始密码'"/></el-form-item><el-form-item label="确认密码"><el-input v-model="userForm.confirmPassword" type="password" show-password :placeholder="editingUser&&!userForm.password?'留空表示不修改密码':'请再次输入密码'"/></el-form-item><el-form-item label="手机号"><el-input v-model="userForm.phone" placeholder="请输入手机号"/></el-form-item><el-form-item label="角色"><el-select v-model="userForm.roleCode" style="width:100%"><el-option label="普通用户" value="USER"/><el-option label="管理员" value="ADMIN"/></el-select></el-form-item><el-form-item label="状态" class="status-form-item"><div class="status-switch"><el-switch v-model="userForm.status" active-value="ACTIVE" inactive-value="DISABLED"/><span>{{userForm.status==='ACTIVE'?'启用':'禁用'}}</span></div></el-form-item></el-form><template #footer><el-button @click="userDialog=false">取消</el-button><el-button type="primary" @click="saveUser">{{editingUser?'保存':'创建'}}</el-button></template></el-dialog>
