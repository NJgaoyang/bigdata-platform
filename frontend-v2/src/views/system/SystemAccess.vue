...
function openEditUser(u:PlatformUser){editingUser.value=u;Object.assign(userForm,{username:u.username,displayName:u.displayName,password:'',roleCode:u.roleCode,status:u.status});userDialog.value=true}
async function saveUser(){try{if(editingUser.value){await accessApi.updateUser(editingUser.value.id,{...userForm});ElMessage.success('用户已更新')}else{await accessApi.createUser({...userForm});ElMessage.success('用户已创建')}userDialog.value=false;editingUser.value=null;await load()}catch(e){ElMessage.error(msg(e))}}
...
<template><div class="ds-page">
  ...
  <div v-else-if="mode==='users'" class="ds-card"><table class="ds-table"><thead><tr><th>用户</th><th>账号</th><th>角色</th><th>状态</th><th>创建时间</th><th>操作</th></tr></thead><tbody><tr v-for="u in users" :key="u.id"><td class="ds-resource">{{u.displayName}}</td><td>{{u.username}}</td><td>{{u.roleCode}}</td><td><StatusBadge :status="u.status"/></td><td>{{fmt(u.createdAt)}}</td><td><template v-if="canEdit"><span class="ds-link" @click="openEditUser(u)">编辑</span><span class="sep">·</span><span class="ds-link" @click="editUserPermissions(u)">模块权限</span><span class="sep">·</span><span class="ds-link" @click="toggle(u)">{{u.status==='ACTIVE'?'禁用':'启用'}}</span><span class="sep">·</span><span class="danger" @click="remove(u)">删除</span></template><span v-else>—</span></td></tr></tbody></table></div>
  ...
  <el-dialog v-model="userDialog" :title="editingUser?'编辑用户':'新增用户'" width="520px"><el-form label-position="top"><el-form-item label="账号"><el-input v-model="userForm.username"/></el-form-item><el-form-item label="显示名称"><el-input v-model="userForm.displayName"/></el-form-item><el-form-item :label="editingUser?'重置密码':'初始密码'"><el-input v-model="userForm.password" type="password" show-password :placeholder="editingUser?'留空表示不修改密码':'请输入初始密码'"/></el-form-item><el-form-item label="角色"><el-select v-model="userForm.roleCode" style="width:100%"><el-option v-for="r in roles" :key="r.id" :label="r.roleName" :value="r.roleCode"/></el-select></el-form-item><el-form-item v-if="editingUser" label="状态"><el-select v-model="userForm.status" style="width:100%"><el-option label="启用" value="ACTIVE"/><el-option label="禁用" value="DISABLED"/></el-select></el-form-item></el-form><template #footer><el-button @click="userDialog=false">取消</el-button><el-button type="primary" @click="saveUser">{{editingUser?'保存':'创建'}}</el-button></template></el-dialog>
